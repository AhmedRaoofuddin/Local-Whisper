...
class ModelStore {
  ...
  mergeModelLists = () => {
    ...
    defaultModels.forEach(defaultModel => {
      ...
      if (existingModelIndex !== -1) {
        ...
        // Force update name, description, and type from default to ensure translations are applied
        if (defaultModel.name && existingModel.origin === ModelOrigin.PRESET) {
          existingModel.name = defaultModel.name;
        }
        if (defaultModel.description && existingModel.origin === ModelOrigin.PRESET) {
          existingModel.description = defaultModel.description;
        }
        if (defaultModel.type && existingModel.origin === ModelOrigin.PRESET) {
          existingModel.type = defaultModel.type;
        }
  ...
  // ModelStore.ts chatTitle computed property
  ...
  get chatTitle() {
      ...
      // Default display when no model is loaded
      if (!this.context) {
        return 'On-Device Sage';
      }

      // Get current active model
      const activeModel = this.activeModel;
      if (activeModel) {
        // Prefer model's name property
        if (activeModel.name) {
          return activeModel.name;
        }

        // If no name property, try to get from metadata
